...
const Filtro: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  const [startDate, setStartDate] = useState<Date | null>(
    filters.startDate ? new Date(filters.startDate) : defaultFilters.startDate,
  );
  const [endDate, setEndDate] = useState<Date | null>(
    filters.endDate ? new Date(filters.endDate) : defaultFilters.endDate,
  );
  ...
  const clampPrice = (value: number) => {
    ...
    return Math.min(1000000, Math.max(0, value));
  };

  const handlePriceChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setPrice(clampPrice(Number(event.target.value)));
  };
    ...
    <div className={`filtro-contenedor ${className}`}>
      <form onSubmit={handleSubmit} className="filtro-formulario-buscador">
        <div className="filtro-search-form">
          <div className="filtro-form-group filtro-form-group-precio">
          ...
          <div className="filtro-form-group">
            <label htmlFor="price" className="filtro-label-range">
              Precio máximo:
            </label>
            <div className="filtro-range-container">
              ...
              />
              <span className="filtro-price-label">${formatPrice(price)}</span>
            </div>
          </div>
          <button type="submit" className="filtro-button-buscar">
            Buscar
          </button>
        </div>
      </form>
