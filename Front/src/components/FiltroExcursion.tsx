...
const FiltroVerticalExcursiones: React.FC<FiltroVerticalExcursionesProps> = ({
  onFiltrar,
}) => {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      ...
      updatedSelectedTipos = updatedSelectedTipos.filter(
        (tipo) => tipo !== value,
      );
    }
          ...
          {tiposExcursion.map((tipoObj, index) => (
            <label className="filtro-excursiones-option" key={index}>
              ...
              <span className="filtro-excursiones-label">
                {tipoObj.tipo}
                <span className="filtro-excursiones-count">
                  ({tipoObj.cantidad})
                </span>
              </span>
            </label>
