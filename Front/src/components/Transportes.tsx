...
const Transportes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (
      !location.state?.transportes ||
      location.state.transportes.length === 0
    ) {
      cargarTransportes();
    }
  }, []);
  ...
  const cargarTransportes = async () => {
    ...
    setError("");
    try {
      const response = await axios.get("/api/paqueteTransporte");
      ...
      setError(
        "Error al cargar los transportes. Por favor, intenta nuevamente.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const transportesFiltrados = transportes.filter((transporte: Transporte) => {
    const cumpleTipo =
      ...
      (transporte.tipoTransporte &&
        filtros.tipos.includes(transporte.tipoTransporte.nombre));

    const cumpleOrigen =
      filtros.ciudadesOrigen.length === 0 ||
      (transporte.ciudadOrigen &&
        filtros.ciudadesOrigen.includes(transporte.ciudadOrigen.nombre));

    const cumpleDestino =
      filtros.ciudadesDestino.length === 0 ||
      (transporte.ciudadDestino &&
        filtros.ciudadesDestino.includes(transporte.ciudadDestino.nombre));
              ...
              <div className="transporte-card" key={transporte.id}>
                <div className="transporte-header">
                  <div className="transporte-type-badge">
                    <span className="transporte-icon">
                      {transporte.tipoTransporte?.nombre === "Avión"
                        ? "✈️"
                        : transporte.tipoTransporte?.nombre === "Colectivo"
                          ? "🚌"
                          : transporte.tipoTransporte?.nombre === "Tren"
                            ? "🚂"
                            : transporte.tipoTransporte?.nombre === "Barco"
                              ? "⛴️"
                              : "🚗"}
                    </span>
                    <span className="transporte-type">
                      {transporte.tipoTransporte?.nombre || "N/A"}
                    </span>
                  </div>
                  <div className="transporte-empresa">
                ...
                <div className="transporte-content">
                  <div className="transporte-name">
                    ...
                    {transporte.tipo === "IDA" ? "Ida" : "Vuelta"}
                  </div>
                  ...
                  <div className="transporte-info">
                    <div className="info-item">
                      ...
                      <span className="info-text">
                        {transporte.capacidad} asientos
                      </span>
                    </div>
                    {transporte.asientos_disponibles != null && (
                      <div className="info-item available">
                        <span className="info-icon">✅</span>
                        <span className="info-text">
                          {transporte.asientos_disponibles} disponibles
                        </span>
                      </div>
                    )}
                    {transporte.fecha_salida && (
                      <div className="info-item">
                        <span className="info-icon">🕒</span>
                        <span className="info-text">
                          {new Date(transporte.fecha_salida).toLocaleString(
                            "es-ES",
                          )}
                        </span>
                      </div>
