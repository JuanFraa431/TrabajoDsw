...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Tipos de transporte encontrados",
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Tipo de transporte encontrado", data: tipoTransporte });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
...
async function create(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: "Tipo de transporte creado", data: tipoTransporte });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
...
async function remove(req: Request, res: Response) {
  try {
    ...
    return handleDatabaseError(error, res, "TipoTransporte");
  }
}
