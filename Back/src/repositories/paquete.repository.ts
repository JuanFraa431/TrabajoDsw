...
export class PaqueteRepository implements Repository<Paquete> {
    ...
    public async save(item: Paquete): Promise<Paquete> {
        const [result] = (await pool.query(
          ...
          [item.estado, item.descripcion, item.precio, item.fecha_ini, item.fecha_fin, item.imagen]
        )) as RowDataPacket[];
    ...
    public async update(item: { id: string }, paquete: Paquete): Promise<Paquete | undefined> {
        ...
        const [result] = (await pool.query(
            ...
            [paquete.estado, paquete.descripcion, paquete.precio, paquete.fecha_ini, paquete.fecha_fin, paquete.imagen, id]
        )) as RowDataPacket[];
        const affectedRows = (result as any).affectedRows
        if (affectedRows == 1) {
            return paquete
