...
export class TransporteRepository implements Repository<Transporte>{
    ...
    public async findAll(): Promise<Transporte [] | undefined> {
        const [transportes] = await pool.query('SELECT * FROM transporte')
        return transportes as Transporte []
    }

    public async findOne(item: {id:string}): Promise<Transporte | undefined>{
        const id = Number.parseInt(item.id)
        const [transportes] = await pool.query<RowDataPacket[]>('SELECT * FROM transporte where id = ?',
            [id])
    ...
    public async save(item: Transporte): Promise<Transporte> {
        const [result] = await pool.query('INSERT INTO transporte (descripcion, capacidad, tipo, nombre_empresa, mail_empresa) VALUES (?, ?, ?, ?, ?)',
    ...
    public async update(item: {id:string}, transporte: Transporte): Promise<Transporte | undefined> {
        ...
        const [result] = (await pool.query(
          'UPDATE transporte SET descripcion = ?, capacidad = ?, tipo = ?, nombre_empresa = ?, mail_empresa = ? WHERE id = ?',
          [
    ...
    public async remove(item: {id:string}): Promise<void> {
        const id = Number.parseInt(item.id)
        const [result] = await pool.query('DELETE FROM transporte WHERE id = ?',
            [id]) as RowDataPacket[]
        const affectedRows = (result as any).affectedRows
        if(affectedRows == 0){
